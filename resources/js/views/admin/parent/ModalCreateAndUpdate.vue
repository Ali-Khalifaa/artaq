<template>
    <div class="modal fade" id="category-service" tabindex="-1" aria-labelledby="adminModalLabel" aria-hidden="true">
...
export default {
    name: "ParentModalCreateAndUpdate",
    props: {
        type: { default: 'create' },
        dataRow: { default: '' },
    },
    ...
    setup(props) {
        ...
        return { t, id, loading, is_disabled, resetModal, empty, preview, resetModalHidden, imageUpload, image, ...toRefs(submitdata), v$, numberOfImage, requiredn, errors };
    },
    methods: {
        AddSubmit() {
            ...
            if (this.type !== 'edit') {
                if (!this.v$.$error) {
                    ...
                    this.loading = true;
                    adminApi.post(`dashboard/parent`, formData)
                        .then((res) => {
                            Swal.fire({
                                icon: 'success',
                ...
                adminApi.post(`dashboard/parent/${this.id}`, formData)
                    .then((res) => {
                        Swal.fire({
                            icon: 'success',
