<template>
    <div class="container-fluid">
        ...
        <div class="d-md-flex d-block align-items-center justify-content-between my-4 page-header-breadcrumb">
            <h1 class="page-title fw-semibold fs-18 mb-0">{{ $t('global.privacies') }}</h1>
            <div class="ms-md-1 ms-0">
                <nav>
                    <ol class="breadcrumb mb-0">
                        <li class="breadcrumb-item"><router-link :to="{name: 'dashboard'}">{{$t('global.home')}}</router-link></li>
                        <li class="breadcrumb-item active" aria-current="page">{{ $t('global.privacies') }}</li>
                    </ol>
                </nav>
            </div>
        </div>
            ...
            <div class="card-body">
                <div class="row">
                     <div class="col-md-12 mt-3">
                        <label class="form-label">{{ $t('global.content') }}</label>
                        <textarea
                            class="form-control summernote"
                            rows="6"
                            v-model.trim="v$.content.$model"
                            :class="{'is-invalid': v$.content.$error ||errors[`content`],
                            'is-valid':!v$.content.$invalid && !errors[`content`]}">
                        </textarea>
                        <div class="invalid-feedback">
                            <span v-if="v$.content.required.$invalid">{{ $t('validation.fieldRequired') }}<br /> </span>
                        </div>
                        <template v-if="errors[`content`]">
                            <error-message v-for="(errorMessage, index) in errors[`content`]" :key="index">
                                {{ errorMessage }}
                            </error-message>
                        </template>
                    </div>
...
export default {
    ...
    setup(){
       ...
       function resetModal() {
            defaultData();
            setTimeout(async () => {
                adminApi.get(`dashboard/privacy/${id.value}`)
                    .then((res) => {
                        loading.value = true;
                        let l = res.data.data;
                        submitdata.data.content = l.content;
                    })
                    .catch((err) => {
                        console.log(err);
                    })
    ...
    methods: {
        ...
        if(!this.v$.$error) {
            this.loading = true;
            adminApi.post(`dashboard/privacy/${this.id}`,formData)
                .then((res) => {
                    Swal.fire({
                        icon: 'success',
