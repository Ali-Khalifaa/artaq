<template>
    <div class="container-fluid">
        ...
        <div class="d-md-flex d-block align-items-center justify-content-between my-4 page-header-breadcrumb">
            <h1 class="page-title fw-semibold fs-18 mb-0">{{ $t('global.policies') }}</h1>
            <div class="ms-md-1 ms-0">
                <nav>
                    <ol class="breadcrumb mb-0">
                        <li class="breadcrumb-item"><router-link :to="{name: 'dashboard'}">{{$t('global.home')}}</router-link></li>
                        <li class="breadcrumb-item active" aria-current="page">{{ $t('global.policies') }}</li>
                    </ol>
                </nav>
            </div>
        </div>
...
export default {
    ...
    setup(){
       ...
       function resetModal() {
            defaultData();
            setTimeout(async () => {
                adminApi.get(`dashboard/policy/${id.value}`)
                    .then((res) => {
                        loading.value = true;
                        let l = res.data.data;
    ...
    methods: {
        ...
        if(!this.v$.$error) {
            this.loading = true;
            adminApi.post(`dashboard/policy/${this.id}`,formData)
                .then((res) => {
                    Swal.fire({
                        icon: 'success',
